...
const getDatesForFilter = (filterType: string) => {
  ...
  console.log(
    `getDatesForFilter: ${filterType} -> Start: ${formatDate(start)}, End: ${formatDate(end)}`,
  );
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        if (startAt && endAt) {
          const salesSummary = await axiosPrivate.get(
            `/list-sales?period=${period}&start_at=${startAt}&end_at=${endAt}`,
          );
          console.log("Sales Summary Data:", salesSummary.data); // Added log
          setSalesData(salesSummary.data.data);
          const sumSales = salesSummary.data.data.reduce(
            (sum: number, item: SalesType) => sum + item.sales,
            0,
          );
          setTotalSales(sumSales);
        } else if (selectedFilter === "all") {
          // If "all" is selected, fetch all sales without date parameters
